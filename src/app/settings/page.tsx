...
import { addHolidayAction, deleteHolidayAction, type HolidayState } from "@/app/actions/settings-actions";
import { db } from '@/lib/firebase/config';
import { collection, query, where, onSnapshot, orderBy, Timestamp } from 'firebase/firestore';
import { format, getYear } from 'date-fns';
import { Calendar as CalendarIcon, PlusCircle, Trash2, Loader2, AlertCircle, Settings as SettingsIcon } from 'lucide-react';
...
export default function SettingsPage() {
  ...
  // Form state for adding a holiday
  ...
  // Effect to show toasts on action completion
  useEffect(() => {
    if (addState?.message) {
      toast({
      ...
      if (addState.success) {
        setAddHolidayForm({ name: "" }); // Reset form
      }
    }
  }, [addState, toast]);
      ...
      <div className="space-y-8">
        <header>
          ...
          <p className="text-muted-foreground">
            Manage company-wide settings like official holidays.
          </p>
        </header>
        ...
        <Card className="shadow-lg">
          <CardHeader>
            ...
            <CardDescription>
              Add or remove official holidays for the company. These days will be excluded from leave duration calculations.
            </CardDescription>
          </CardHeader>
                  ...
                  <div className="space-y-2">
                    ...
                    <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
                      <PopoverTrigger asChild>
                        <Button
                          variant={"outline"}
                          className={cn("w-full justify-start text-left font-normal", !addHolidayForm.date && "text-muted-foreground")}
                        >
                      ...
                      <PopoverContent className="w-auto p-0">
                        <Calendar
                          mode="single"
                          selected={addHolidayForm.date}
                          onSelect={date => {
                            setAddHolidayForm(prev => ({ ...prev, date }));
